...
# 最后的random noise data是和类别相关的，不同的类是不同的variance，一样的mean生成的gaussian noise
...
def main():
    ...
    if args.data_aug == 0:
        ...
        train_transform, valid_transform = utils._random_noise_transforms_cifar10_fake_random(args)
    ...
    elif args.data_aug == 11:
        train_transform, valid_transform = utils._flip_data_transforms_fracDB(args)
        print("Data aug is weak gaussian noise")
    # train_data = generateCIFAR.randomNoiseCIFAR100GenLabels(root=args.data, train=True, download=True, transform=train_transform,
    #                                              corrupt_prob=args.label_corrupt_prob, num_classes=args.num_class)
    #
    # val_data = generateCIFAR.randomNoiseCIFAR100GenLabels(root=args.data, train=True, download=True, transform=valid_transform,
    #                                              corrupt_prob=args.label_corrupt_prob, num_classes=args.num_class)

    train_data = generateCIFAR.CIFAR100GenLabels(root=args.data, train=True, download=True,
                                                            transform=train_transform,
                                                            corrupt_prob=args.label_corrupt_prob,
                                                            num_classes=args.num_class)

    val_data = generateCIFAR.CIFAR100GenLabels(root=args.data, train=True, download=True,
    ...
    for epoch in range(args.epochs + args.warm_up_epochs):
        ...
        logging.info('reduce_alphas = %s', F.softmax(model.alphas_normal, dim=-1))
